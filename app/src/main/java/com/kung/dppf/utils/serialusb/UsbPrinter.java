package com.kung.dppf.utils.serialusb;
            ...
            for (int i = 0; i < usbInterface.getEndpointCount(); i++) {
                ...
                if (ep.getType() == UsbConstants.USB_ENDPOINT_XFER_BULK) {
                    if (ep.getDirection() == UsbConstants.USB_DIR_OUT) {
                        int pid = mUsbDevice.getProductId();    //产品ID
                        int vid = mUsbDevice.getVendorId(); //厂商ID
...
//                bytes = new byte[]{27, 97, (byte) 2};
    ...
    public void go() { write(feedPaperCutAll());}
    ...
    public static byte[] feedPaperCutAll()
    {
        ...
        public void command(byte[] orders){
            write(orders);
        }
...
//        }
    ...
    public int sendMessage(byte[] bytes) {
        return mUsbDeviceConnection.bulkTransfer(usbEndpointOut, bytes, bytes.length, 500);
    }
